...
/** One platform setting: a switch for booleans (flips at once), a field + Save for the rest. */
export function PlatformSettingRow({ setting }: { setting: { key: string; value: unknown; description: string; updated_at: string } }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 220px', gap: 12, alignItems: 'center', padding: '10px 14px', background: 'var(--surface)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--r-md)', fontSize: 13 }}>
      ...
      {isBool ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'flex-end' }}><span style={{ color: 'var(--app-text-muted)' }}>{setting.value ? 'On' : 'Off'}</span><Switch on={setting.value as boolean} onChange={(on) => setPlatformSetting(setting.key, on)} label={setting.key} /></div>
      ) : (
        <div style={{ display: 'flex', gap: 6 }}>
          <input className="ops-input" value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') void run(() => setPlatformSetting(setting.key, parse(draft)), `${setting.key} saved`) }} />
          <button className="btn btn-primary btn-sm" data-busy={busy} disabled={busy || draft === shown} onClick={() => void run(() => setPlatformSetting(setting.key, parse(draft)), `${setting.key} saved`)}>{busy ? <span className="spinner" /> : 'Save'}</button>
        </div>
      )}
    </div>
  )
...
export function OpsSettingsForm({ fields, settings, updatedAt }: { fields: { key: string; label: string; help: string; unit: string }[]; settings: Record<string, number>; updatedAt: string }) {
  ...
  useEffect(() => setDraft(Object.fromEntries(fields.map((f) => [f.key, String(settings[f.key] ?? '')]))), [settings, fields])
  const { busy, run } = useAct()
  ...
  return (
    <div style={{ display: 'grid', gap: 8, maxWidth: 720 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <button className="btn btn-primary" data-busy={busy} disabled={busy || invalid || n === 0} onClick={() => void run(() => patchOperationalSettings(changed), 'Saved — terminals receive it on next sync')}>{busy ? <><span className="spinner" /> Saving…</> : n ? `Save ${n}` : 'Save'}</button>
