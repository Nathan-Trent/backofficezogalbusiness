...
/** The grid. Each switch flips at once and confirms in place; the row arrives back by push. */
export function NotifyGrid({ staff, events, rows }: { staff: { id: string; email: string; name: string | null; role: string }[]; events: EventDef[]; rows: Row[] }) {
  const get = (s: string, e: string): Row => rows.find((r) => r.staff_id === s && r.event === e) ?? { staff_id: s, event: e, email: false, bell: false }
                ...
                return (
                  <td key={s.id} style={{ padding: '10px 12px', textAlign: 'center' }}>
                    <span style={{ display: 'inline-flex', gap: 8 }}>
                      <Switch on={r.bell} onChange={(on) => setNotify(s.id, e.key, 'bell', on)} label={`${s.email} bell ${e.label}`} />
                      <Switch on={r.email} onChange={(on) => setNotify(s.id, e.key, 'email', on)} label={`${s.email} email ${e.label}`} />
                    </span>
                  </td>
                )
              })}
