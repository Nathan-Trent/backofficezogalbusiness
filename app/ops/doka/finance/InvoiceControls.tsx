  ...
  return (
    <>
      ...
      <ConfirmDialog open={open} wide title="Raise an invoice" confirmLabel={pending ? 'Raising…' : `Raise ₦${Number(amount).toLocaleString()} and email`} busy={pending} onCancel={() => setOpen(false)}
        onConfirm={() => go(async () => { const r = await createInvoice({ shopId: shop, planKey: plan, months, startDate: start }); if (r.ok) { toast.success(`Invoice ${(r.data as { number: string }).number} raised and emailed`); setOpen(false); router.refresh() } else toast.error(r.error) })}
...
export function InvoiceActions({ id, status }: { id: string; status: string }) {
  ...
  const [pending, go] = useTransition()
  const done = (msg: string) => { toast.success(msg); setPaying(false); setVoiding(false); router.refresh() }
